...
function extractNameFromTemplate(value) {
    let match = value.match(/^Hello, ([a-z, ]+)/i);
...
function getRectangleString(width, height) {
    ...
    for (var i = 0; i < height; i++) {
        if (i == 0) (width > 2) ? output += '┌' + repeatString('─', width-2) + '┐\n' : output += '┌┐\n';
        else if (i == (height - 1)) (width > 2) ? output += '└' + repeatString('─', width-2) + '┘\n' : output += '└┘\n';
        else output += '│' + repeatString(' ', width-2) + '│\n';
    }
    return output;
}

/**
 * Move char code 13 positions and transform to char
 *
 * @param {number} from
 * @param {number} to
 * @param {number} code
 * @return {string}
*/
function moveChar(from, to, code) {
    code += 13;
    if (code > to) code = from + (code-(to + 1));
...
function encodeToRot13(str) {
    return str.split('').map(function(ch) {
        ...
        if (code >= 65 && code <= 90) {
            return moveChar(65, 90, code);
        } else if (code >= 97 && code <= 122) {
            return moveChar(97, 122, code);
        } else {
            return ch;
        }
    }).join('');
...
function getCardId(value) {
    let decks_m = {'9827':0, '9830':13, '9829':26, '9824':39};
    let s_m = {'A':1, 'J':11, 'Q':12, 'K':13};
    let l = value.length;
    let deck = value[l-1];
    let n = value.replace(deck, '');
    let index = decks_m[deck.charCodeAt()] + ((n.match(/^\d+/) !== null) ? eval(n) : s_m[n]);
    return index-1;
}
